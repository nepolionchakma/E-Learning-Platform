...
function InfoBox({ type, content }: { type: string; content: string }) {
  ...
  return (
    <div className={`my-4 p-4 rounded-lg border ${styles[type] || styles.info}`}>
      ...
      <table className="w-full text-xs sm:text-sm">
        <thead>
          <tr className="bg-zinc-100 dark:bg-zinc-800">
            {headers.map((h, i) => (
              <th key={i} className="p-3 text-left font-semibold text-zinc-700 dark:text-zinc-300 whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className="border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
              {row.map((cell, ci) => (
...
function ContentItem({ item }: { item: any }) {
  ...
  if (item.text) {
    return (
      <div className="mb-4">
        {item.label && <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-2">{item.label}</p>}
        <div className="text-sm text-zinc-600 dark:text-zinc-400 space-y-2">
          {typeof item.text === 'string' ? <p>{item.text}</p> : item.text.map((line: string, idx: number) => <p key={idx}>{line}</p>)}
        </div>
      </div>
    );
  }
  if (item.table) {
    return (
      <div className="mb-4 overflow-x-auto rounded-lg border border-zinc-200 dark:border-zinc-700">
        {item.label && <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 mb-2 p-3">{item.label}</p>}
        <table className="w-full text-xs sm:text-sm">
          <thead>
            <tr className="bg-zinc-100 dark:bg-zinc-800">
              {item.table.headers.map((header: string, idx: number) => (
                <th key={idx} className="border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-left font-semibold">{header}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {item.table.rows.map((row: string[], ridx: number) => (
              <tr key={ridx} className="border-t border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800">
                {row.map((cell: string, cidx: number) => (
                  <td key={cidx} className="border border-zinc-200 dark:border-zinc-700 px-3 py-2">{cell}</td>
                ))}
              </tr>
            ))}
          </tbody>
  ...
  return (
    <section id={section.id} className="mb-8 sm:mb-12 scroll-mt-24">
      ...
      {section.content?.map((p, i) => (
        <p key={i} className="mb-4 text-sm sm:text-base">{p}</p>
      ))}

      {/* infoBox */}
      {section.infoBox && (
        <InfoBox type={section.infoBox.type} content={section.infoBox.content} />
      )}
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 mb-4">
          {section.cards.map((card, i) => (
            <div key={i} className="p-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900">
              <h4 className="font-semibold text-zinc-900 dark:text-zinc-100 mb-1 text-sm sm:text-base">{card.title}</h4>
              <p className="text-xs sm:text-sm text-zinc-600 dark:text-zinc-400">{card.desc}</p>
            </div>
          ))}
        </div>
      )}
      ...
      {section.image && (
        <div className="mb-6 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-700">
          <img src={section.image.src} alt={section.image.alt} className="w-full h-auto" />
        </div>
      )}
      ...
      {/* tables */}
      {section.tables && <DataTable headers={section.tables.headers} rows={section.tables.rows} />}
      {section.targets && <DataTable headers={section.targets.headers} rows={section.targets.rows} />}
      {section.optionsTable && <DataTable headers={section.optionsTable.headers} rows={section.optionsTable.rows} />}
      {section.criteriaTable && <DataTable headers={section.criteriaTable.headers} rows={section.criteriaTable.rows} />}
      {section.summaryTable && <DataTable headers={section.summaryTable.headers} rows={section.summaryTable.rows} />}

      {/* packetFlow */}
      {section.packetFlow && (
        <div className="mb-4 p-4 rounded-lg bg-zinc-900 text-green-400 font-mono text-xs sm:text-sm space-y-1">
          {section.packetFlow.map((line, i) => <div key={i}>{line}</div>)}
        </div>
      )}
      ...
      {section.subsections?.map((sub: any, ssi: number) => (
        <div key={ssi} className="mt-6 sm:mt-8 ml-0 sm:ml-2">
          {sub.title && (
            <h3 className="text-base sm:text-lg font-semibold text-zinc-900 dark:text-zinc-100 mb-3">{sub.title}</h3>
          )}
          ...
          <div className={`mb-4 sm:mb-6 rounded-xl sm:rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-gradient-to-br ${heroColor} p-12 sm:p-16`}>
            <div className="flex items-center justify-center h-64 sm:h-80">
              <div className="text-center">
                {data.icon && <div className="text-6xl sm:text-7xl mb-4">{data.icon}</div>}
                <p className="text-white text-lg sm:text-xl font-semibold">{data.title.split(':')[0]}</p>
              </div>
            </div>
          </div>
        )}
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-zinc-900 dark:text-zinc-100 leading-tight">{data.title}</h1>
        <p className="text-zinc-500 mt-2 sm:mt-3 text-sm sm:text-base lg:text-lg">{data.description}</p>
      ...
      <div className="flex gap-6 lg:gap-8">
        ...
        <aside className="hidden lg:block w-56 xl:w-64 flex-shrink-0">
          <div className="sticky top-24 max-h-[calc(100vh-8rem)] overflow-y-auto">
            <h3 className="text-xs sm:text-sm font-semibold text-zinc-900 dark:text-zinc-100 uppercase tracking-wide mb-3">Table of Contents</h3>
